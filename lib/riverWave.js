...
/**
 * Same vertical weight as former `getSectionBottomRiverMaskSvgDataUri` (amp 96 in viewBox height 1000),
 * scaled by `capScale` (= dividerCapRem / 2.75).
 */
const SECTION_CLIP_WAVE_DEPTH = 96 / 1000

/**
 * `points` for `<polygon clipPathUnits="objectBoundingBox">` — wavy **bottom** (0–1 coords only).
 * Avoids CSS `clip-path: polygon(calc…)` / mask data-URIs, which break in several engines.
 */
export function getSectionBottomClipBBoxPolygonPoints(variant = 'classic', steps = 80, capScale = 1) {
  const yBb = (xu) => {
    const yPath = riverPresetGoldY(xu, variant)
    const factor = 1 - yPath / RIVER_VIEWBOX_H
    return 1 - SECTION_CLIP_WAVE_DEPTH * capScale * factor
  }
  const pts = ['0,0', '1,0']
  for (let i = steps; i >= 0; i--) {
    const xu = (i / steps) * 1200
    const x = (xu / 1200).toFixed(6)
    pts.push(`${x},${yBb(xu).toFixed(6)}`)
  }
  pts.push('0,0')
  return pts.join(' ')
}
...
/**
 * `points` for `<polygon clipPathUnits="objectBoundingBox">` — wavy **top** (delivery / footer).
 */
export function getSectionTopClipBBoxPolygonPoints(variant = 'classic', steps = 80, capScale = 1) {
  const yBb = (xu) => {
    const yPath = riverPresetGoldY(xu, variant)
    const factor = yPath / RIVER_VIEWBOX_H
    return SECTION_CLIP_WAVE_DEPTH * capScale * factor
  }
  const pts = [`0,${yBb(0).toFixed(6)}`]
