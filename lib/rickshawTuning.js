...
export const RICKSHAW_TUNING_DEFAULTS = {
  ...
  travelStartPercent: -500,
  /** Horizontal end position (same units) */
  travelEndPercent: 40,
  /** Seconds for one full crossing */
  travelDurationSec: 5.2,
  /**
   * Controls line visibility (alpha boost). Higher = bolder/darker lines.
   * Range: ~0.3 (faint) to 1.5 (very bold). Default 0.85.
   */
  outlineDilateRadius: 3.0,
}

/** Bumped when shipped defaults change so stale slider saves don't override them. */
export const RICKSHAW_TUNING_STORAGE_KEY = 'bangla-kitchen-rickshaw-tuning-v8'
...
export function mergeRickshawTuning(patch = {}) {
  ...
  return {
    ...
    outlineDilateRadius: clamp(m.outlineDilateRadius, 0.01, 3.0, RICKSHAW_TUNING_DEFAULTS.outlineDilateRadius),
  }
}
