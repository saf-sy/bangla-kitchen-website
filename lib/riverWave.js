...
/** Y on the gold wave at x ∈ [0, 1200] (svg user units). */
export function riverGoldWaveY(x) {
  const xx = Math.min(1200, Math.max(0, x))
  for (const seg of GOLD_SEGMENTS) {
    if (xx <= seg.x1 + 1e-9) {
...
/**
 * clip-path for dark section top: wavy upper edge follows {@link riverGoldWaveY} / gold stroke.
 * `capRem` must match RiverDivider render height (Tailwind `h-11` = 2.75rem, `md:h-[3.75rem]` = 3.75rem).
 */
export function getContactTopWaveClipPolygonRem(capRem, steps = 96) {
  const pts = []
  for (let i = 0; i <= steps; i++) {
    const xu = (i / steps) * 1200
    const yPath = riverGoldWaveY(xu)
...
/**
 * Closed SVG path: area below RIVER_WAVE_GOLD_D down to floorY (viewBox coords).
 * Same wave sampling as strokes — use for filled caps (e.g. menu top) so there’s no transparent band.
 */
export function getRiverWaveFillCapPathD(floorY60 = 60, steps = 96) {
  let d = `M 0 ${riverGoldWaveY(0).toFixed(3)}`
  for (let i = 1; i <= steps; i++) {
    const x = (i / steps) * 1200
    d += ` L ${x.toFixed(2)} ${riverGoldWaveY(x).toFixed(3)}`
  }
